...
def process_pdf(pdf_path: str, start_page: int, end_page: int, vector_store_path: str, index_name: str = "default") -> VectorStore:
    """
...
def generate_questions(vector_store: VectorStore, 
                       num_topics: int = 12,
                       questions_per_topic: int = 3,
                       top_k: int = 4,
                       difficulty_range: Optional[List[float]] = None) -> tuple[List[Dict[str, Any]], Dict[str, str]]:
    """
    ...
    if not difficulty_range:
        ...
        step = (max_difficulty - min_difficulty) / (len(topics) - 1) if len(topics) > 1 else 0
    ...
    for i, (topic, difficulty) in enumerate(zip(topics, difficulty_range)):
        logger.info(f"Generating questions for topic: {topic} (difficulty: {difficulty:.1f})")
...
def evaluate_questions(questions: List[Dict[str, Any]], contexts: Dict[str, str]) -> Dict[str, Any]:
    """
    ...
    logger.info(f"Evaluation complete. Overall quality score: {stats['avg_overall']:.2f}/10")
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description="RAG Multiple Choice Question Generator")
    parser.add_argument('--pdf_path', type=str, default=os.path.join("data", "ISLRv2.pdf"),
                       help="Path to the PDF file")
    parser.add_argument('--start_page', type=int, default=int(os.environ.get("PDF_START_PAGE", "14")),
                       help="First page to process (0-indexed)")
    parser.add_argument('--end_page', type=int, default=int(os.environ.get("PDF_END_PAGE", "597")),
                       help="Last page to process (0-indexed)")
    parser.add_argument('--output_file', type=str, default="questions.json",
    ...
    try:
        ...
        save_questions_to_file(results, args.output_file.replace(".json", "_evaluated.json"))
        logger.info("=== Question Evaluation Step Completed ===")
        
        logger.info(f"Pipeline completed successfully. Files saved with base name: {args.output_file}")
