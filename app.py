...
def load_vector_store(vector_store_path: str, index_name: str = "islr_index") -> Optional[VectorStore]:
    """
    ...
    """
    try:
        # Initialize vector store with allow_dangerous_deserialization=True
        # This is safe since we're loading our own files
...
def generate_questions_for_topic(
    ...
    if vector_store is None or not hasattr(vector_store, 'vector_store') or vector_store.vector_store is None:
        st.error("Vector store is not properly initialized. Please run the main.py script first.")
        return []
    ...
    if len(choices) > 0:
        ...
        if selected_answer:
            ...
            if selected_answer == correct_answer:
                ...
                st.error(f"❌ Incorrect. The correct answer is {correct_answer}: {choices.get(correct_answer, '')}")
                st.info(f"Explanation: {explanation}")
            
            return {"answered": True, "selected": selected_answer, "correct": selected_answer == correct_answer}
...
def main():
    ...
    vector_store_exists = os.path.exists(os.path.join(vector_store_path, "islr_index"))
    ...
    if generate_btn:
        with st.spinner("Generating questions... This may take a moment."):
            try:
                ...
                if not vector_store:
                    st.error("Failed to load vector store. Please check logs for details.")
                    return
                ...
                if not questions:
                    st.warning("No questions could be generated. Try a different topic or settings.")
                    return
                ...
                # Success message
                st.success(f"Generated {len(questions)} questions for {selected_topic} ({selected_level} level)")
            except Exception as e:
    ...
    if hasattr(st.session_state, 'questions') and st.session_state.questions:
        ...
        if 0 <= current_idx < len(questions):
            ...
            st.progress(progress, text=f"Answered {len(st.session_state.answered_questions)} of {len(questions)} questions")
